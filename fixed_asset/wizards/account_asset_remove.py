...
class AccountAssetRemove(orm.TransientModel):
            ...
            if line.invoice_id.state in ["open", "paid"]:
                ...
                if inv_curr != comp_curr:
                    amount = currency_obj.compute(
                        cr, uid, inv_curr.id, comp_curr.id, amount, context=context
                    )
                sale_value += amount
    ...
    _columns = {
        ...
        "account_sale_id": fields.many2one(
            "account.account", "Asset Sale Account", domain=[("type", "=", "other")]
        ),
        "account_plus_value_id": fields.many2one(
            "account.account", "Plus-Value Account", domain=[("type", "=", "other")]
        ),
        "account_min_value_id": fields.many2one(
            "account.account", "Min-Value Account", domain=[("type", "=", "other")]
        ),
        "account_residual_value_id": fields.many2one(
            "account.account", "Residual Value Account", domain=[("type", "=", "other")]
        ),
    ...
    def _prepare_early_removal(
        ...
        if to_depreciate_amount:
            ...
            asset_line_obj.create_move(cr, uid, [dl_ids[0]], context=context)
            dl_ids.pop(0)
    ...
    def _get_removal_data(
        ...
        if residual_value:
            if wiz_data.posting_regime == "residual_value":
                ...
                account_id = (
                    wiz_data.account_plus_value_id.id
                    if balance > 0
